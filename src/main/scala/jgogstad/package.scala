...
package object jgogstad {
  ...
  implicit class DenseMatrixOps[A](matrix: DenseMatrix[A]) {
    ...
    def convolveTap[S, B: Zero](
      mask: DenseMatrix[B]
    )(f: ((Int, Int), A, List[((Int, Int), A)]) => A): DenseMatrix[A] = {
      convolveAcc(mask, ())((_, c, a, m) => () -> f(c, a, m))._2
      matrix
    }
    ...
    def convolveAcc[S, B: Zero](mask: DenseMatrix[B], z: S)(
      ...
      val s = matrix.activeKeysIterator.foldLeft(z) { case (acc, ij @ (i, j)) =>
        ...
        val window: DenseMatrix[A] = matrix(
          clampRows to Math.min(matrix.rows - 1, i + mask.rows / 2),
          clampCols to Math.min(matrix.cols - 1, j + mask.cols / 2)
        )
        ...
        val realCoordinates = list.mapFilter { case (c, (_, include)) =>
